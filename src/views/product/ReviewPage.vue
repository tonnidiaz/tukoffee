<template>
        <ion-page>
        <Appbar title="Write a review" :show-cart="false"/> 
...
import { errorHandler, showAlert, sleep } from "@/utils/funcs";
const product = ref<Obj>();
const formRef = ref<HTMLFormElement>()
const form = ref<Obj>({
    "name": "Tonni",
    "title": "Dope mate",
    "body": "Yoh this is some real deal",
    rating: 3.5
});
...
const submitReview = async (e: any) => { 
    ...
    if( !formRef.value?.checkValidity()) {
        ...
        return};
    try{
        const res = await apiAxios.post('/products/review?act=add', {pid: product.value!.pid, review: _form})

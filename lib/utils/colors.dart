import 'package:flutter/material.dart';

const Color titlebarBG = Color.fromRGBO(66, 66, 66, 1);
const appBGLight =
    Color.fromRGBO(245, 245, 245, 1); //Color.fromRGBO(238, 238, 238, 1);
const cardBG = Color.fromRGBO(39, 39, 39, 1);
const cardBG2 = Color.fromRGBO(32, 32, 32, 1);
const grey = Color.fromRGBO(42, 42, 42, 1);
const Color cardBGLight =
    Color.fromARGB(255, 255, 255, 255); //Color.fromRGBO(48, 48, 48, 1);
const Color appBG = Color.fromRGBO(29, 29, 29, 1);
const Color bg0 = Color.fromRGBO(46, 46, 46, 1);
const Color bg1 = Color.fromRGBO(56, 56, 56, 1);
const Color fg1 = Color.fromRGBO(0, 0, 0, .6);
const Color fg2 = Color.fromRGBO(112, 112, 112, 1);
const Color orange =
    Color.fromRGBO(255, 152, 0, 1); // Color.fromRGBO(225, 106, 6, 1);
const Color btnBG = Color.fromRGBO(45, 45, 45, 1);
const Color imgBG = Color.fromRGBO(228, 228, 228, 1);

class TuColors {
  static Color coffee = const Color.fromRGBO(63, 35, 5, 1);
  static Color coffee1 = const Color.fromRGBO(108, 52, 40, 1);
  static Color coffee2 = const Color.fromRGBO(186, 112, 79, 1);
  static Color coffee3 = const Color.fromRGBO(223, 168, 120, 1);

  //static Color fieldBG = const Color.fromRGBO(255, 153, 0, .1);
  static Color fieldBG = const Color.fromRGBO(196, 196, 196, 0.2);
  //static Color fieldBG = Color.fromARGB(255, 255, 203, 124);
}
